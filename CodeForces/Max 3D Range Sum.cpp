...
void solve() {
    ...
    vector<vector<vector<int>>> v2(n+1,vector<vector<int>>(n+1,vector<int>(n+1)));
    for(int i =1; i<= n; i++){
        for(int j =1; j <= n; j++){
            for(int k =1; k <= n; k++){
                cin>> v[i][j][k];
    ...
    for(int i =1; i<= n; i++){
        for(int j =1; j <= n; j++){
            for(int k =1; k <= n; k++){
                v2[i][j][k] = v[i][j][k] + v2[i][j-1][k] + v2[i][j][k-1] - v2[i][j-1][k-1];
            }
        }
    }
    ll maxi = -LLINF; 
    for (int i = 1; i <= n; i++){
        for (int j = i; j <= n; j++){
            for (int k = 1; k <= n; k++){
                for (int l = k; l <= n; l++){
                    long long maxia = -LLINF; 
                    long long suma = 0;
                    for (int m = 1; m <= n; m++){
                        long long val = v2[m][j][l] - v2[m][i - 1][l] - v2[m][j][k - 1] + v2[m][i - 1][k - 1];
                        if(suma > 0){
                            suma += val;
                        }else{
                            suma = val;
